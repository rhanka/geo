/**
 * _zones-vnatif-deposit-agglo-longueuil-20260810.ts — DÉPÔT v2 (ESTABLISH) des zones
 * NATIVES de brossard + boucherville depuis le SIG de l'Agglomération de Longueuil.
 *
 * SOURCE (découverte 2026-08-10) : ArcGIS Server ON-PREM de la Ville/Agglo de Longueuil
 *   https://geomatique.longueuil.quebec/public/rest/services/CarteInteractive/
 *     Amenagement_du_territoire/MapServer/3  (couche « Zonage », polygones)
 * La couche est PARTAGÉE pour toute l'agglo ; le champ GROUPEUSAGE discrimine la muni
 * (VLO/GPK/STH = arrondissements de Longueuil ; BRO = Brossard ; BOU = Boucherville ;
 * STB = Saint-Bruno ; STL = Saint-Lambert). On filtre par where=GROUPEUSAGE=<CODE> et on
 * capture f=geojson (outSR=4326). Le zone_code = valeur BRUTE du champ AUTORITAIRE de la
 * couche `ZONAGEMUNICIPALID` (ex. brossard "Pl-608"/"Hl-*"/"Cl-*", boucherville "A-2004"/
 * "C-671"/"H-324"). DISPOSITIONSPECIALE pointe vers .../hotlink/Logo/Zonage/<CODE>/<id>.pdf,
 * corroborant la nature « zonage » per-muni.
 *
 * DIFFÉRENCE avec la recette AGOL (5564b2f5 victoriaville / 0b343380 altus / _deposit-agol) :
 *   - Cibles UNPROUVÉES (servi legacy-traceable, zone_source_url=null, AUCUNE preuve v2).
 *     Un v2 frais ÉTABLIT la géométrie servie ; il n'EXIGE donc PAS ≥90% de recouvrement
 *     avec le servi (l'overlap est CALCULÉ et RAPPORTÉ, jamais opposé comme garde). Le
 *     gate d'identité PROVENANCE-AWARE de depositCapturedZones ne bloque que sur un code
 *     servi RÉELLEMENT prouvé v2 (aucun ici) ; les codes servi-seulement → UNKNOWN + backup.
 *   - Champ code-zone FIXE = ZONAGEMUNICIPALID (champ autoritaire de la couche amont),
 *     jamais résolu par recouvrement. Valeur BRUTE ; 0 valeur vide toléré (anti-invention).
 *   - Preuve type = "arcgis" (ArcGIS Server on-prem), method=natif, reliability=directe.
 *
 * GARDES conservées (par muni, isolation stricte : un muni KO = SKIP) :
 *   G2 byte-exact (rehash CAS == manifeste == clé CAS + verifyRawCapturePayload) ;
 *   FC non vide ; 100% polygonal ; anti-homonyme nearest_registre_muni===slug ;
 *   grain=zone-polygon (aucun marqueur UEV) ; ZONAGEMUNICIPALID présent & 0 vide ;
 *   anti-troncature ArcGIS (returnCountOnly[MÊME where]==features & !exceededTransferLimit) ;
 *   servi ne porte PAS déjà une vraie preuve v2 ; servi EXISTE (upgrade, pas création aveugle).
 *   G5 readback : géométrie octet-exacte de la capture, level→documented, url=proof.url,
 *   geometry_grain=zone-polygon, backup _replaced/ présent, dropped→UNKNOWN.
 *
 * `--dry-run` (défaut) : resolve + G2 + grain + count + overlap SANS déposer.
 * `--commit`           : dépose réellement.
 * `--only <slug>`      : restreint à un muni.
 * `--worklist f.json`  : worklist source (défaut brossard-boucherville).
 * `--out f.json`       : écrit le record.
 *
 * USAGE :
 *   NODE_OPTIONS=--dns-result-order=ipv4first AWS_MAX_ATTEMPTS=10 \
 *   npx tsx acquisition/src/_zones-vnatif-deposit-agglo-longueuil-20260810.ts \
 *     [--commit] [--only <slug>] --out work/coverage/zones-vnatif-deposit-record-brossard-boucherville-20260810.json
 */
...
const RUN_STAMP = "20260810T230000Z";
const GRAIN: GeometryGrain = "zone-polygon";
const CODE_FIELD = "ZONAGEMUNICIPALID"; // champ AUTORITAIRE de la couche amont (fixe, jamais résolu).
const DEFAULT_WORKLIST = "work/coverage/zones-vnatif-capture-worklist-brossard-boucherville-20260810.json";
...
// Comptage LIVE : conserve LE MÊME filtre `where` (GROUPEUSAGE=<code>) que l'URL geojson capturée.
...
/** Résout le NOM EXACT du champ code-zone sur les features (exact puis insensible à la casse). */
function resolveCodeFieldName(feats: Feat[]): string | null {
  const keys = new Set<string>();
  for (const f of feats) for (const k of Object.keys(f.properties ?? {})) keys.add(k);
  if (keys.has(CODE_FIELD)) return CODE_FIELD;
  const ci = [...keys].find((k) => k.toUpperCase() === CODE_FIELD.toUpperCase());
  return ci ?? null;
}
...
// Le manifeste enregistre `redactUrlForManifest(urlWorklist).url`. On matche donc l'URL
// worklist RAW, sa forme rédigée/normalisée, ET final_url — pour rester robuste à toute
// normalisation d'URL sans jamais confondre deux munis (le where GROUPEUSAGE=<code> diffère).
...
async function processMuni(
  ...
  const entry: Record<string, unknown> = {
    ...
    code_field: CODE_FIELD, expected_geometry_grain: GRAIN, zone_code_source: `champ autoritaire ${CODE_FIELD} (valeur brute)`,
    deposit_mode: "ESTABLISH (servi UNPROUVÉ → v2 frais ; overlap RAPPORTÉ, non opposé)",
  };
  try {
    ...
    // Champ code-zone AUTORITAIRE (fixe) + valeurs brutes
    const codeField = resolveCodeFieldName(feats);
    ...
    // overlap servi → capture (RAPPORTÉ, non opposé comme garde : cible unprouvée = ÉTABLISSEMENT)
    const uncovered = codeField ? [...served.codes].filter((c) => !capCodes.has(c)).sort() : [...served.codes].sort();
    ...
    entry.served_only_codes_to_unknown = uncovered.length;
    ...
    // ── Gardes ──
    const guards: string[] = [];
    if (served.keys.length === 0) guards.push("aucun servi (rien à établir/upgrader — hors périmètre de cette passe)");
    ...
    if (!codeField) guards.push(`champ code-zone autoritaire ${CODE_FIELD} absent des features`);
    if (codeField && nullCode > 0) guards.push(`${nullCode} feature(s) sans zone_code (champ ${CODE_FIELD} vide) — anti-invention`);
    ...
    if (!commit) {
      ...
      entry.raison = `toutes gardes OK — prêt au dépôt ESTABLISH v2 (champ=${CODE_FIELD}, ${capCodes.size} codes capture, overlap servi ${String(overlap)}% RAPPORTÉ, ${uncovered.length} code(s) servi-seulement→UNKNOWN, grain=${grain}) (relancer avec --commit)`;
      return entry;
    }
    // ── Dépôt ──
    const norm = normalize(feats as never[], codeField!, url, "obscura-zone-vector");
    ...
    entry.raison = entry.readback_ok
      ? `ESTABLISH v2 byte-exact (arcgis/natif/directe) ; zone_code = ${CODE_FIELD} brut ; grain=${grain} ; overlap servi ${String(overlap)}% ; ${droppedCodes.length} code(s) servi-seulement→UNKNOWN ; backup _replaced/ présent`
...
async function main(): Promise<void> {
  ...
  const record = {
    contract: "zones-vnatif-deposit-record-brossard-boucherville/v1",
    date: "2026-08-10",
    spec: "SPEC_ZONE_DEPOSIT_REPLACE_POLICY.md + SPEC_ZONE_GEOMETRY_GRAIN.md + SPEC_ACQUISITION_METHODES_PAR_SOURCE.md §12 ; réplique dépôt agol/victoriaville 5564b2f5 / altus 0b343380 (variante ESTABLISH cible-unprouvée, champ code fixe)",
    decision: "ESTABLISH v2 brossard+boucherville depuis le SIG Agglomération de Longueuil (ArcGIS Server on-prem geomatique.longueuil.quebec, couche Zonage MapServer/3 PARTAGÉE filtrée GROUPEUSAGE=BRO|BOU, f=geojson outSR=4326), zone_code brut du champ autoritaire ZONAGEMUNICIPALID, grain zone-polygon, preuve arcgis/natif/directe",
    mode: commit ? "commit" : "dry-run",
    run_stamp: RUN_STAMP,
    worklist: worklistPath,
    source_layer: "https://geomatique.longueuil.quebec/public/rest/services/CarteInteractive/Amenagement_du_territoire/MapServer/3",
    code_field: CODE_FIELD,
    proof_intended: { type: "arcgis", method: "natif", reliability: "directe", schema_version: "2.0" },
    gate: "depositCapturedZones identity gate PROVENANCE-AWARE (discriminateur = preuve v2 par-feature) + geometry_grain + anti-troncature ArcGIS (returnCountOnly[MÊME where]==features & !exceededTransferLimit) + anti-homonyme nearest==slug ; overlap RAPPORTÉ non opposé (ÉTABLISSEMENT d'une cible unprouvée)",
