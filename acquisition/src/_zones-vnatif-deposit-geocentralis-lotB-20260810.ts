/**
 * _zones-vnatif-deposit-geocentralis-lotB-20260810.ts — DÉPÔT v2 candidate/legacy-traceable
 * → documented du LOT-B géoCentralis WFS (~40 munis, 2 couches partagées), grain zone-polygon.
 *
 * Réplique EXACTE de la recette pilote (05de001a/cd363a04) et du lot-A (656eb46d) à l'échelle.
 * Source = geoserver.geocentralis.com/geoserver/ows, GetFeature outputFormat=application/json
 * filtré par CQL_FILTER=id_municipalite=<id>, DÉJÀ CAPTURÉ sur le cluster (run 20260810T160000Z).
 *
 * Les munis + URLs viennent de la worklist émise par la sonde lot-A (byte-exact : l'URL de la
 * worklist == l'URL rédigée du manifeste == l'URL de findLine). La couche est parsée depuis
 * typeNames= et mappée vers son champ-code :
 ...
 * `--worklist f.json`  : worklist source (défaut lot-A).
 * `--out f.json`       : écrit le record.
 *
 * USAGE :
 *   NODE_OPTIONS=--dns-result-order=ipv4first AWS_MAX_ATTEMPTS=10 \
 *   npx tsx acquisition/src/_zones-vnatif-deposit-geocentralis-lotB-20260810.ts \
 *     [--commit] [--only <slug>] --out work/coverage/zones-vnatif-deposit-record-geocentralis-lotB-20260810.json
 */
...
const RUN_STAMP = "20260810T160000Z";
const GRAIN: GeometryGrain = "zone-polygon";
const OVERLAP_MIN_PCT = 90;
const DEFAULT_WORKLIST = "work/coverage/zones-vnatif-capture-worklist-geocentralis-lotB-20260810.json";
...
async function main(): Promise<void> {
  ...
  const record = {
    contract: "zones-vnatif-deposit-record-geocentralis-lotB/v1",
    date: "2026-08-10",
    ...
    decision: "CLEAN-UPGRADE lot-B géoCentralis WFS (legacy-traceable/candidate→documented v2), source-identity WFS, zone_code brut du champ-code (réplique pilote 05de001a/cd363a04 + lot-A 656eb46d)",
