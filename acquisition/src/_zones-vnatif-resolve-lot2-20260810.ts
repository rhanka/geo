/**
 * _zones-vnatif-resolve-lot2-20260810.ts — SONDE DIAGNOSTIC (lecture seule, aucun dépôt).
 *
 * Variante lot2 de _zones-vnatif-resolve-20260810.ts (run-stamp 20260810T110000Z,
 * 3 villes vecteur natif : lassomption, saint-colomban, sainte-sophie).
 *
 * Pour la capture cluster run-stamp 20260810T110000Z :
 *   1. liste `capture/_runs/zones-20260810T110000Z-*`, dérive les run_id (shards) ;
 *   2. pour chaque run VÉRIFIE le header (finished_at + exit_code 0) ;
 *   3. pour chaque URL de la worklist lot2, localise la ligne de capture (url|final_url)
 ...
 *      choisir le zone_field réel (SKIP si code composite multi-champ, cf mirabel).
 *
 * N'écrit RIEN sur S3. Émet un rapport JSON sur stdout.
 *
 * USAGE :
 *   NODE_OPTIONS=--dns-result-order=ipv4first AWS_MAX_ATTEMPTS=10 \
 *   npx tsx acquisition/src/_zones-vnatif-resolve-lot2-20260810.ts
 */
...
const RUN_STAMP = "20260810T110000Z";
const LANE = "zones";
const PREFIX = `capture/_runs/${LANE}-${RUN_STAMP}-`;
const WORKLIST = "work/coverage/zones-vnatif-capture-worklist-lot2-20260810.json";
...
async function main(): Promise<void> {
  requireS3();
  const worklist = JSON.parse(readFileSync(resolve(ROOT, WORKLIST), "utf8")) as WorklistEntry[];
  ...
  for (const w of worklist) {
    ...
    // Candidate zone fields: every non-housekeeping property (exclude FID/OID/Shape/geom metrics).
    const candidates = [...allKeys].filter((k) => !/^(fid|objectid(_1)?|shape_?_?(area|leng?|le_1)|shape__(area|length)|lien)$/i.test(k));
    ...
    // Deep-dive saint-colomban: COMBINE well-formedness vs ORI/NO_ZONE, LAYER counts.
    if (w.slug === "saint-colomban") {
      const layerCounts: Record<string, number> = {};
      const combineWhenNoZoneEmpty: string[] = [];
      let combineMatchesOriNoZone = 0, noZonePresent = 0;
      const combineWellFormed = /^[A-Za-zÀ-ÿ]+\d*-?[\w./]*$/;
      let wellFormed = 0;
      const combineDistinct = new Set<string>();
      for (const f of feats) {
        const p = (f.properties ?? {}) as Record<string, unknown>;
        const layer = String(p["LAYER"] ?? "").trim() || "(empty)";
        layerCounts[layer] = (layerCounts[layer] ?? 0) + 1;
        const combine = String(p["COMBINE"] ?? "").trim();
        const ori = String(p["ORI"] ?? "").trim();
        const noZone = String(p["NO_ZONE"] ?? "").trim();
        if (combine) { combineDistinct.add(combine); if (combineWellFormed.test(combine)) wellFormed++; }
        if (noZone === "") { if (combineWhenNoZoneEmpty.length < 40) combineWhenNoZoneEmpty.push(combine || "(empty)"); }
        else { noZonePresent++; if (combine === `${ori}-${noZone}`) combineMatchesOriNoZone++; }
      }
      entry.deep = {
        layer_counts: layerCounts,
        combine_distinct: combineDistinct.size,
        combine_wellformed: wellFormed,
        combine_wellformed_pct: Math.round((wellFormed / feats.length) * 1000) / 10,
        no_zone_present: noZonePresent,
        combine_eq_ori_dash_nozone_when_nozone_present: combineMatchesOriNoZone,
        combine_when_nozone_empty: combineWhenNoZoneEmpty,
      };
    }
    report.push(entry);
  }
