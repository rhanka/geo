/**
 * _zones-vnatif-deposit-victoriaville-20260810.ts — DÉPÔT v2 candidate/legacy-traceable
 * → documented du batch VICTORIAVILLE (couche ArcGIS MapServer PARTAGÉE
 * `IntranetMRC/ZonageMunicipal/MapServer/3` sur `geo.victoriaville.ca`, HONORE
 * f=geojson, filtrée par muni via `Code_mun`), grain zone-polygon, type de preuve
 * = arcgis / natif / directe.
 *
 * Réplique EXACTE de la recette de dépôt ALTUS (commit 0b343380) elle-même réplique
 * de géoCentralis lot-D — G2 byte-exact, grain, anti-homonyme, gate PROVENANCE-AWARE,
 * level→documented, url=proof.url, backup _replaced/, dropped→UNKNOWN (jamais N-A),
 * readback G5. SEULES DIFFÉRENCES avec ALTUS :
 *   (1) SOURCE = UNE couche MapServer PARTAGÉE filtrée par muni
 *       `<layer>/query?where=Code_mun='<code>'&outFields=*&f=geojson` (au lieu d'une
 *       couche par muni avec where=1=1). Preuve type=arcgis (comme altus).
 ...
 *       ET une requête de comptage LIVE portant LE MÊME filtre Code_mun
 *       `<layer>/query?where=Code_mun='<code>'&returnCountOnly=true&f=json`
 *       → { count } == features (numberReturned==numberMatched).
 *   (3) CHAMP CODE-ZONE : la couche partagée porte `Disposition_spéciale` (code-zone
 *       ex "AF8"/"RU13") ET `GROUPEUSAGE` (AFFECTATION ex "AF"/"R" — À ÉVITER). Le
 *       champ EXACT est RÉSOLU par muni, jamais deviné : parmi les champs non-techniques,
 *       sans valeur vide, ≥3 codes distincts, on retient celui qui REPRODUIT ≥90% des
 *       codes DÉJÀ SERVIS (legacy). Le recouvrement avec la vérité-terrain servie EST le
 *       discriminateur (au scoping : Disposition_spéciale=100%, GROUPEUSAGE=0%). zone_code
 *       = valeur BRUTE du champ résolu (aucune dérivation synthétique).
 *
 ...
 * `--worklist f.json`  : worklist source (défaut victoriaville).
 * `--out f.json`       : écrit le record.
 *
 * USAGE :
 *   NODE_OPTIONS=--dns-result-order=ipv4first AWS_MAX_ATTEMPTS=10 \
 *   npx tsx acquisition/src/_zones-vnatif-deposit-victoriaville-20260810.ts \
 *     [--commit] [--only <slug>] --out work/coverage/zones-vnatif-deposit-record-victoriaville-20260810.json
 */
...
const RUN_STAMP = "20260810T200000Z";
const OVERLAP_MIN_PCT = 90;
const DEFAULT_WORKLIST = "work/coverage/zones-vnatif-capture-worklist-victoriaville-20260810.json";
// Champs jamais éligibles comme code-zone (géométrie / id / audit / admin-muni de la
// couche partagée victoriaville). Comparés en forme CANONIQUE (canonName = alnum-uppercase),
// donc `Code_mun`→CODEMUN, `Shape.STArea()`→SHAPESTAREA, etc.
...
// Comptage LIVE : conserve LE MÊME filtre `where` (Code_mun='<code>') que l'URL geojson.
...
async function main(): Promise<void> {
  ...
  const record = {
    contract: "zones-vnatif-deposit-record-victoriaville/v1",
    date: "2026-08-10",
    spec: "SPEC_ZONE_DEPOSIT_REPLACE_POLICY.md + SPEC_ZONE_GEOMETRY_GRAIN.md + SPEC_ACQUISITION_METHODES_PAR_SOURCE.md §12 ; réplique dépôt altus 0b343380",
    decision: "CLEAN-UPGRADE batch victoriaville (legacy-traceable/candidate→documented v2), source ArcGIS MapServer PARTAGÉE filtrée par Code_mun, f=geojson, champ code-zone résolu par recouvrement ≥90% avec le servi (Disposition_spéciale attendu ; GROUPEUSAGE=affectation évitée), zone_code brut",
    mode: commit ? "commit" : "dry-run",
    run_stamp: RUN_STAMP,
    worklist: worklistPath,
    proof_intended: { type: "arcgis", method: "natif", reliability: "directe", schema_version: "2.0" },
    gate: "depositCapturedZones identity gate PROVENANCE-AWARE (discriminateur = preuve v2 par-feature) + geometry_grain + anti-troncature ArcGIS (returnCountOnly[where=Code_mun]==features & !exceededTransferLimit)",
