/**
 * _zones-vnatif-deposit-otherhttp-20260810.ts — DÉPÔT v2 legacy-traceable → documented
 * du BATCH "other-http / petit-hôte" — la veine MARGINALE finale de la campagne v2-upgrade.
 * Deux sources ArcGIS REST natives, hors des 4 veines-plateforme déjà traitées :
 *   (1) sig.mrcal.ca — FeatureServer PARTAGÉ `EVALUATION/sde_zonage_s/FeatureServer/0`
 *       (MRC Antoine-Labelle), Polygon, HONORE f=geojson, filtré par muni via le champ
 *       `code` (code géographique MAMH ; ex 79065). Champ code-zone = `zonage`.
 *   (2) carte.rouyn-noranda.ca — MapServer mono-muni `Donnees_ouvertes/.../MapServer/5`
 *       ("Plan de zonage", Ville de Rouyn-Noranda, MUNICIPALITE=86042), Polygon, f=geojson,
 *       where=1=1. Champ code-zone = `NO_ZONE`.
 * Preuve type = arcgis / natif / directe. Grain zone-polygon.
 *
 * Réplique EXACTE de la recette de dépôt VICTORIAVILLE (commit 5564b2f5) / ALTUS /
 * géoCentralis lot-D — G2 byte-exact, grain, anti-homonyme (nearest==slug), gate
 * PROVENANCE-AWARE, level→documented, url=proof.url, backup _replaced/, dropped→UNKNOWN
 * (jamais N-A), readback G5, anti-troncature ArcGIS (returnCountOnly[MÊME where]==features
 * & !exceededTransferLimit), champ code-zone RÉSOLU par recouvrement ≥90% avec le servi
 * (valeur BRUTE, aucune dérivation). Aucune règle métier neuve : sonde de dépôt jetable.
 *
 * SEULE DIFFÉRENCE avec victoriaville : deux hôtes/couches distincts pilotés par la worklist
 * (mrcal partagé filtré par `code` ; rouyn mono-muni where=1=1) + `code`/`nm_pdf_rgl`
 * (identité-muni de la couche mrcal partagée) ajoutés aux champs techniques exclus.
 *
 ...
 * `--worklist f.json`  : worklist source (défaut otherhttp).
 * `--out f.json`       : écrit le record.
 *
 * USAGE :
 *   NODE_OPTIONS=--dns-result-order=ipv4first AWS_MAX_ATTEMPTS=10 \
 *   npx tsx acquisition/src/_zones-vnatif-deposit-otherhttp-20260810.ts \
 *     [--commit] [--only <slug>] --out work/coverage/zones-vnatif-deposit-record-otherhttp-20260810.json
 */
...
const RUN_STAMP = "20260811T000000Z";
const OVERLAP_MIN_PCT = 90;
const DEFAULT_WORKLIST = "work/coverage/zones-vnatif-capture-worklist-otherhttp-20260810.json";
// Champs jamais éligibles comme code-zone (géométrie / id / audit / identité-muni des
// couches sources). Comparés en forme CANONIQUE (canonName = alnum-uppercase), donc
// `Code_mun`→CODEMUN, `nm_pdf_rgl`→NMPDFRGL, `Shape.STArea()`→SHAPESTAREA, etc.
// `CODE`+`NMPDFRGL` = code géographique muni de la couche mrcal partagée (JAMAIS un code-zone).
...
const KNOWN_CODE_NAMES = new Set<string>(["ZONE", "ZONAGE", "NUMZONE", "NOZONE", "DISPOSITIONSPECIALE"]);
...
// Comptage LIVE : conserve LE MÊME filtre `where` que l'URL geojson.
...
async function processMuni(
  ...
  try {
    ...
    // Parse ArcGIS geojson — un payload non-JSON (page HTML) = la source ne sert PAS
    // le geojson complet sur cette requête (ex. carte.rouyn-noranda.ca : la requête
    // where=1=1 pleine étendue ~48 Mo dépasse le délai de la passerelle en frontal et
    // renvoie une page HTML d'application au lieu du geojson). SKIP explicite, jamais ERROR.
    const looksHtml = line.storage_key.endsWith(".html") || /^\s*<(?:!doctype|html)/i.test(bytes.toString("utf8", 0, 64));
    if (looksHtml) {
      entry.captured_content = "html-not-json";
      entry.statut = "SKIP";
      entry.raison = "capture non-JSON (page HTML) — la source ne sert pas de geojson complet sur cette requête (requête pleine étendue tronquée/timeout passerelle en frontal) ; capture complète byte-exact single-payload impossible → hors pipeline standard";
      return entry;
    }
    let gj: { type?: string; features?: Feat[]; exceededTransferLimit?: unknown; error?: unknown };
    try {
      gj = JSON.parse(bytes.toString("utf8")) as { type?: string; features?: Feat[]; exceededTransferLimit?: unknown; error?: unknown };
    } catch (parseErr) {
      entry.captured_content = "non-json";
      entry.statut = "SKIP";
      entry.raison = `capture non parsable en JSON (${(parseErr as Error).message.slice(0, 80)}) — source ne sert pas de geojson exploitable`;
      return entry;
    }
    ...
    if (!commit) {
      ...
      entry.raison = `toutes gardes OK — prêt au dépôt legacy-traceable→documented (champ=${chosen}, overlap ${String(overlap)}%, ${uncovered.length} code(s) servi-seulement→UNKNOWN, grain=${grain}) (relancer avec --commit)`;
      return entry;
    }
    // ── Dépôt ──
...
async function main(): Promise<void> {
  ...
  const record = {
    contract: "zones-vnatif-deposit-record-otherhttp/v1",
    date: "2026-08-10",
    spec: "SPEC_ZONE_DEPOSIT_REPLACE_POLICY.md + SPEC_ZONE_GEOMETRY_GRAIN.md + SPEC_ACQUISITION_METHODES_PAR_SOURCE.md §12 ; réplique dépôt victoriaville 5564b2f5",
    decision: "CLEAN-UPGRADE veine marginale other-http (legacy-traceable→documented v2). Deux sources ArcGIS REST natives hors 4 veines-plateforme : (1) sig.mrcal.ca FeatureServer PARTAGÉ (MRC Antoine-Labelle) filtré par `code` (code géo MAMH), champ code-zone `zonage` ; (2) carte.rouyn-noranda.ca MapServer mono-muni (Plan de zonage) where=1=1, champ code-zone `NO_ZONE`. f=geojson, champ code-zone résolu par recouvrement ≥90% avec le servi, zone_code brut",
    mode: commit ? "commit" : "dry-run",
    run_stamp: RUN_STAMP,
    worklist: worklistPath,
    proof_intended: { type: "arcgis", method: "natif", reliability: "directe", schema_version: "2.0" },
    gate: "depositCapturedZones identity gate PROVENANCE-AWARE (discriminateur = preuve v2 par-feature) + geometry_grain + anti-troncature ArcGIS (returnCountOnly[MÊME where]==features & !exceededTransferLimit)",
