/**
 * _zones-vnatif-record-md-geocentralis-lotD-20260810.ts — génère le record .md LOT-D
 * géoCentralis à partir du record .json machine (jamais reformater la sortie à la main).
 *
 * USAGE :
 *   npx tsx acquisition/src/_zones-vnatif-record-md-geocentralis-lotD-20260810.ts \
 *     --in work/coverage/zones-vnatif-deposit-record-geocentralis-lotD-20260810.json \
 *     --out work/coverage/zones-vnatif-deposit-record-geocentralis-lotD-20260810.md \
 *     [--job-complete 12/12]
 */
...
function main(): void {
  ...
  const jobComplete = arg("job-complete") ?? "N/N";
  ...
  const md = `# Dépôt zones — LOT-D géoCentralis WFS — 2026-08-10
...
pilote (\`05de001a\`/\`cd363a04\`) + lot-A (\`656eb46d\`) + lot-B (\`dd7e4cf9\`) + lot-C (\`bf4afa98\`). **${rec.summary.deposited} DÉPÔTS EFFECTUÉS**,
...
Worklist : \`work/coverage/zones-vnatif-capture-worklist-geocentralis-lotD-20260810.json\` (${rec.summary.total} munis).
...
- Job \`geo-capture-zones-${rec.run_stamp.toLowerCase()}\` : **Complete ${jobComplete}**, run-stamp \`${rec.run_stamp}\`, 0 shard en échec.
...
Lot-D géoCentralis **CLÔTURÉ : ${rec.summary.deposited}/${rec.summary.total} DÉPOSÉS, ${rec.summary.skipped} SKIP** (gardes légitimes). Chemin WFS → preuve v2
