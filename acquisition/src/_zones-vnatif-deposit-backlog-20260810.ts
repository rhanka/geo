/**
 * _zones-vnatif-deposit-backlog-20260810.ts — DÉPÔT v2 candidate/legacy-traceable →
 * documented du BACKLOG récupéré de la file SKIP de la campagne upgrade v2.
 *
 * Réplique EXACTE de la recette de dépôt géoCentralis lot-B (dd7e4cf9) / AGOL (5cc7e680) —
 * G2 byte-exact, grain, gate PROVENANCE-AWARE, level→documented, url=proof.url, backup
 * _replaced/, dropped→UNKNOWN (jamais N-A), readback G5, anti-troncature. DEUX ajouts
 * CONTRÔLÉS, chacun anti-invention, jamais forcé :
 *
 *  (A) OVERRIDE ANTI-HOMONYME sur IDENTITÉ FORTE (per-muni, explicite dans BACKLOG_CFG) :
 *      le garde `nearest==slug` est un FAUX-NÉGATIF pour un muni rural adjacent quand une
 *      identité PLUS FORTE existe. Deux modes, tous deux VÉRIFIÉS ici, jamais supposés :
 *        - "wfs-id-verified" : le filtre WFS `id_municipalite=<id>` DOIT égaler le
 *          mamhCode du slug (qc-municipal-directory.json) ET le mamhCode du muni-nearest
 *          flaggé DOIT être DIFFÉRENT de <id> (donc l'id ne pointe pas le voisin). L'id
 *          est PARSÉ de l'URL capturée, jamais du config.
 *        - "agol-name-match" : le filtre ArcGIS `MUNI='<Nom>'` DOIT correspondre (NFD) au
 *          `name` du slug dans municipalities.qc.json. Le nom est PARSÉ de l'URL.
 *      Dans les DEUX cas, l'overlap source-identity ≥90% reste EXIGÉ. Sinon le garde tient.
 *
 *  (B) DROP EMPTY-CODE (per-muni, explicite `allow_empty_drop`) : quelques features à code
 *      vide sont RETIRÉES ssi (a) la fraction vide ≤ EMPTY_DROP_MAX_FRAC ET (b) l'overlap
 *      du sous-ensemble non-vide reste ≥90%. Les zones à code vide NE SONT PAS servies —
 *      jamais fabriquer un code. La preuve couvre les octets bruts complets (sha inchangé) ;
 *      la géométrie SERVIE est le sous-ensemble codé (prouvé octet-pour-octet, sous-ensemble
 *      du téléchargement vérifié complet par l'anti-troncature). Le drop est ENREGISTRÉ.
 *
 * Type de preuve par source : geoserver.geocentralis.com→wfs ; services*.arcgis.com→agol ;
 * gis.altusquebec.com→arcgis. Champ code-zone : WFS = fixe par couche ; ArcGIS = résolu par
 * recouvrement ≥90% (valeur BRUTE, jamais dérivée).
 *
 * `--dry-run` (défaut) | `--commit` | `--only <slug>` | `--worklist f.json` | `--out f.json`.
 *
 * USAGE :
 *   NODE_OPTIONS=--dns-result-order=ipv4first AWS_MAX_ATTEMPTS=10 \
 *   npx tsx acquisition/src/_zones-vnatif-deposit-backlog-20260810.ts \
 *     [--commit] [--only <slug>] --out work/coverage/zones-vnatif-deposit-record-backlog-20260810.json
 */
...
import { proofFromCaptureEntry, type GeometryGrain, type GeometrySourceType } from "./lib/zonage-proof.js";
...
const RUN_STAMP = "20260810T220000Z";
const OVERLAP_MIN_PCT = 90;
const EMPTY_DROP_MAX_FRAC = 0.1; // ≤10% de features à code vide peuvent être retirées (minorité minime)
const DEFAULT_WORKLIST = "work/coverage/zones-vnatif-capture-worklist-backlog-20260810.json";
...
const KNOWN_CODE_NAMES = new Set<string>(["ZONE", "ZONAGE", "NUMZONE", "NOZONE", "SECT", "SECTLABEL", "USAGE", "ETIQUETTE1", "NOZONAGEMUNICIPAL"]);
const CODE_FIELD_BY_LAYER: Record<string, string> = {
  "evb:zonage_municipal": "no_zonage_municipal",
  "evb:siadmin_pzon_99_s": "etiquette_1",
};

type IdentityOverride = "wfs-id-verified" | "agol-name-match" | null;
interface BacklogCfg { identity_override: IdentityOverride; allow_empty_drop: boolean; note: string }
// Décision per-muni (anti-invention : override EXPLICITE + vérifié ; drop EXPLICITE + borné).
const BACKLOG_CFG: Record<string, BacklogCfg> = {
  // (A) anti-homonyme WFS id-vérifié
  "pointe-aux-outardes": { identity_override: "wfs-id-verified", allow_empty_drop: false, note: "anti-homonyme faux-négatif (nearest=chute-aux-outardes, muni rural adjacent) — id 96030 = mamhCode" },
  "saint-anaclet-de-lessard": { identity_override: "wfs-id-verified", allow_empty_drop: false, note: "anti-homonyme faux-négatif (nearest=saint-donat--la-mitis) — id 10030 = mamhCode" },
  "saint-camille-de-lellis": { identity_override: "wfs-id-verified", allow_empty_drop: false, note: "anti-homonyme faux-négatif (nearest=saint-just-de-bretenieres) — id 28070 = mamhCode" },
  "saint-gabriel": { identity_override: "wfs-id-verified", allow_empty_drop: true, note: "anti-homonyme faux-négatif (nearest=saint-gabriel-de-brandon) — id 52080 = mamhCode ; +1 code vide droppé" },
  "saint-ignace-de-loyola": { identity_override: "wfs-id-verified", allow_empty_drop: false, note: "anti-homonyme faux-négatif (nearest=la-visitation-de-lile-dupas) — id 52045 = mamhCode" },
  "saint-marcel": { identity_override: "wfs-id-verified", allow_empty_drop: false, note: "anti-homonyme faux-négatif (nearest=sainte-apolline-de-patton) — id 17020 = mamhCode" },
  "saint-marcellin": { identity_override: "wfs-id-verified", allow_empty_drop: false, note: "anti-homonyme faux-négatif (nearest=saint-gabriel-de-rimouski) — id 10025 = mamhCode" },
  // (B) empty-code drop (nearest OK)
  "saint-charles-garnier": { identity_override: null, allow_empty_drop: true, note: "2 codes vides droppés (nearest=slug OK)" },
  "saint-donat--la-mitis": { identity_override: null, allow_empty_drop: true, note: "1 code vide droppé (nearest=slug OK)" },
  "beaupre": { identity_override: null, allow_empty_drop: true, note: "1/78 code vide droppé (nearest=slug OK)" },
  // AGOL saint-ludger : anti-homonyme nom-exact
  "saint-ludger": { identity_override: "agol-name-match", allow_empty_drop: false, note: "anti-homonyme faux-négatif (nearest=saint-robert-bellarmin) — filtre MUNI='Saint-Ludger' nom EXACT" },
};
...
function proofTypeForUrl(url: string): GeometrySourceType {
  const host = new URL(url).host;
  if (host === "geoserver.geocentralis.com") return "wfs";
  if (/^services\d*\.arcgis\.com$/.test(host)) return "agol";
  if (host === "gis.altusquebec.com") return "arcgis";
  throw new Error(`type de preuve inconnu pour host ${host}`);
}
function isWfsUrl(url: string): boolean { return new URL(url).host === "geoserver.geocentralis.com"; }
function wfsLayerOf(url: string): string { const m = /typeNames=(evb:[a-z0-9_]+)/i.exec(url); return m ? m[1]! : ""; }
function wfsIdOf(url: string): string | null { const m = /id_municipalite=(?:%27|')?([^&'%]+)/i.exec(url); return m ? m[1]! : null; }
function agolNameOf(url: string): string | null { const m = /where=MUNI(?:=|%3D)(?:%27|')([^&'%]+)/i.exec(decodeURIComponent(url)); return m ? m[1]! : null; }

// Comptage LIVE : conserve LE MÊME filtre. WFS → &count=1 → numberMatched ; ArcGIS → returnCountOnly.
function wfsCountUrl(url: string): string { return `${url}&count=1`; }
function arcgisCountUrl(geojsonUrl: string): string {
...
// ── Registre + directory (anti-homonyme + id-verify) ────────────────────────────
...
function loadRegistry(): MuniEntry[] {
  ...
  return arr.map((m) => ({ slug: String(m["slug"] ?? ""), name: String(m["name"] ?? ""), lat: Number(m["lat"] ?? m["latitude"]), lon: Number(m["lon"] ?? m["lng"] ?? m["longitude"]) }))
...
async function liveCount(url: string): Promise<{ count: number | null; error: string | null }> {
  try {
    if (isWfsUrl(url)) {
      const gj = await fetchJson(wfsCountUrl(url)) as { numberMatched?: unknown };
      return { count: typeof gj.numberMatched === "number" ? gj.numberMatched : null, error: null };
    }
    const gj = await fetchJson(arcgisCountUrl(url)) as { count?: unknown; error?: unknown };
...
// ── Résolution du champ code-zone par recouvrement (ArcGIS) ─────────────────────
interface FieldStat { field: string; distinct: number; null_count: number; overlap_pct: number | null }
function resolveCodeField(feats: Feat[], servedCodes: Set<string>, allowNulls: boolean): { chosen: string | null; stats: FieldStat[] } {
  ...
  const eligible = stats.filter((s) => (allowNulls || s.null_count === 0) && s.distinct >= 3 && s.overlap_pct !== null && s.overlap_pct >= OVERLAP_MIN_PCT);
  eligible.sort((a, b) =>
...
function overlapOf(feats: Feat[], field: string, servedCodes: Set<string>): { overlap: number | null; capCodes: Set<string>; nullCount: number } {
  const capCodes = new Set<string>(); let nullCount = 0;
  for (const f of feats) { const raw = f.properties?.[field]; const s = raw === null || raw === undefined ? "" : String(raw).trim(); if (!s) nullCount++; else capCodes.add(canon(s)); }
  const covered = servedCodes.size > 0 ? [...servedCodes].filter((c) => capCodes.has(c)).length : 0;
  const overlap = servedCodes.size > 0 ? Math.round((covered / servedCodes.size) * 1000) / 10 : null;
  return { overlap, capCodes, nullCount };
}
...
async function processMuni(
  ...
  const url = cfg.url;
  const bcfg = BACKLOG_CFG[cfg.slug] ?? { identity_override: null, allow_empty_drop: false, note: "(config absente)" };
  const proofType = proofTypeForUrl(url);
  const entry: Record<string, unknown> = {
    slug: cfg.slug, run_stamp: RUN_STAMP, source_url: url, proof_type: proofType,
    identity_override: bcfg.identity_override, allow_empty_drop: bcfg.allow_empty_drop, backlog_note: bcfg.note,
  };
  try {
    ...
    // Parse geojson (FULL download)
    const gj = JSON.parse(bytes.toString("utf8")) as { type?: string; features?: Feat[]; exceededTransferLimit?: unknown; numberMatched?: unknown; numberReturned?: unknown; error?: unknown };
    const feats = Array.isArray(gj.features) ? gj.features : [];
    const isFC = gj.type === "FeatureCollection";
    const exceeded = gj.exceededTransferLimit === true;
    const nmCap = typeof gj.numberMatched === "number" ? gj.numberMatched : null;
    const nrCap = typeof gj.numberReturned === "number" ? gj.numberReturned : null;
    entry.is_featurecollection = isFC; entry.feature_count = feats.length; entry.exceeded_transfer_limit = exceeded;
    entry.wfs_number_matched = nmCap; entry.wfs_number_returned = nrCap;
    if (gj && typeof gj === "object" && "error" in gj && gj.error) { entry.statut = "SKIP"; entry.raison = `capture = objet d'erreur (${JSON.stringify(gj.error).slice(0, 120)})`; return entry; }
    ...
    // ── Résolution du champ code-zone ──
    let chosen: string | null;
    if (isWfsUrl(url)) {
      const layer = wfsLayerOf(url); chosen = CODE_FIELD_BY_LAYER[layer] ?? null; entry.wfs_layer = layer;
    } else {
      const r = resolveCodeField(feats, served.codes, bcfg.allow_empty_drop);
      chosen = r.chosen; entry.field_candidates = r.stats;
    }
    entry.code_field_chosen = chosen;
    if (!chosen) { entry.statut = "SKIP"; entry.raison = `aucun champ code-zone résolu (allowNulls=${bcfg.allow_empty_drop})`; return entry; }

    // ── Empty-code drop (borné) OU garde anti-invention stricte ──
    const full = overlapOf(feats, chosen, served.codes);
    entry.full_null_count = full.nullCount; entry.full_overlap_pct = full.overlap;
    let working: Feat[] = feats; let droppedEmpty = 0;
    if (bcfg.allow_empty_drop) {
      working = feats.filter((f) => { const raw = f.properties?.[chosen!]; return raw !== null && raw !== undefined && String(raw).trim() !== ""; });
      droppedEmpty = feats.length - working.length;
      const frac = feats.length > 0 ? droppedEmpty / feats.length : 1;
      entry.dropped_empty_code_count = droppedEmpty; entry.dropped_empty_frac = Math.round(frac * 1000) / 10;
      if (frac > EMPTY_DROP_MAX_FRAC) { entry.statut = "SKIP"; entry.raison = `empty-drop refusé: fraction vide ${(frac * 100).toFixed(1)}% > ${EMPTY_DROP_MAX_FRAC * 100}% (pas une minorité minime)`; return entry; }
      // Anti-loss : le servi legacy RETIENT les features à code vide. Droper les vides réduirait
      // le nombre de features servies — depositCapturedZones REFUSE toute perte de features
      // (coverage gate). Déposer la feature vide exigerait un code FABRIQUÉ (anti-invention).
      // Les deux issues sont interdites → SKIP légitime (irrécupérable sous byte-exact+anti-loss).
      if (droppedEmpty > 0 && working.length < served.features) {
        entry.statut = "SKIP";
        entry.raison = `empty-drop bloqué par coverage-gate anti-perte: le servi retient ${served.features} features (dont ${droppedEmpty} à code vide) ; déposer ${working.length}<${served.features} serait une perte de features ; garder la feature vide exigerait un code fabriqué (anti-invention) → irrécupérable`;
        return entry;
      }
    } else {
      if (full.nullCount > 0) { entry.statut = "SKIP"; entry.raison = `${full.nullCount} feature(s) sans zone_code (champ ${chosen} vide) — anti-invention (drop non autorisé pour ce muni)`; return entry; }
    }
    // overlap du sous-ensemble de travail
    const work = overlapOf(working, chosen, served.codes);
    entry.working_features = working.length; entry.working_overlap_pct = work.overlap; entry.working_null_count = work.nullCount;
    const capCodes = work.capCodes;

    // ── Géométrie / grain / polygonal (sur le sous-ensemble servi) ──
    const geomTypes = new Set<string>();
    for (const f of working) { const gt = (f.geometry as { type?: string } | null)?.type; if (typeof gt === "string") geomTypes.add(gt); }
    ...
    for (const f of working) for (const k of Object.keys(f.properties ?? {})) propKeys.add(k);
    ...
    // ── Anti-homonyme + OVERRIDE d'identité forte ──
    const near = nearestMuni(working, registry);
    entry.nearest_registre_muni = near.slug; entry.registry_attribution_km = near.km;
    let nearestOk = near.slug === cfg.slug; entry.nearest_matches_slug = nearestOk;
    let overrideApplied = false; let overrideBasis: string | null = null;
    if (!nearestOk && bcfg.identity_override === "wfs-id-verified") {
      const id = wfsIdOf(url); const mamhSlug = directory[cfg.slug] ?? null; const mamhNear = near.slug ? (directory[near.slug] ?? null) : null;
      const idPad = id ? String(id).padStart(5, "0") : null;
      const slugOk = idPad !== null && mamhSlug !== null && idPad === mamhSlug;
      const nearDiff = mamhNear === null || mamhNear !== idPad;
      entry.id_from_url = id; entry.mamh_code_slug = mamhSlug; entry.mamh_code_nearest = mamhNear;
      if (slugOk && nearDiff) { overrideApplied = true; overrideBasis = `wfs id_municipalite=${idPad} == mamhCode[${cfg.slug}]=${mamhSlug} ET mamhCode[nearest ${String(near.slug)}]=${String(mamhNear)} != ${idPad} → nearest est un muni rural adjacent (faux-négatif centroïde)`; }
      else entry.override_rejected = `id=${String(idPad)} mamh[slug]=${String(mamhSlug)} slugOk=${slugOk} nearDiff=${nearDiff}`;
    } else if (!nearestOk && bcfg.identity_override === "agol-name-match") {
      const name = agolNameOf(url); const regName = registry.find((r) => r.slug === cfg.slug)?.name ?? null;
      const nameOk = name !== null && regName !== null && normName(name) === normName(regName);
      entry.name_from_url = name; entry.registry_name = regName;
      if (nameOk) { overrideApplied = true; overrideBasis = `agol filtre MUNI='${String(name)}' == name[${cfg.slug}]='${String(regName)}' (NFD) → identité nom-exact prime le centroïde-nearest (nearest ${String(near.slug)} = muni rural adjacent)`; }
      else entry.override_rejected = `name='${String(name)}' regName='${String(regName)}' nameOk=${nameOk}`;
    }
    entry.identity_override_applied = overrideApplied; entry.identity_override_basis = overrideBasis;
    if (overrideApplied) nearestOk = true;

    // ── Anti-troncature (sur le TÉLÉCHARGEMENT COMPLET) ──
    const live = await liveCount(url);
    entry.live_count = live.count; entry.live_count_error = live.error;
    let countComplete: boolean;
    if (isWfsUrl(url)) {
      const noTrunc = nmCap !== null && nrCap !== null && nrCap === nmCap && feats.length === nmCap;
      countComplete = noTrunc && live.count !== null && feats.length === live.count;
      entry.count_complete_basis = `WFS numberReturned(${String(nrCap)})==numberMatched(${String(nmCap)})==features(${feats.length}) && live numberMatched(${String(live.count)})==features`;
    } else {
      countComplete = !exceeded && live.count !== null && feats.length === live.count;
    ...
    // overlap servi → capture (sous-ensemble de travail)
    ...
    if (working.length === 0) guards.push("sous-ensemble de travail vide après drop");
    if (!allPolygonal) guards.push("géométrie non 100% polygonale");
    if (!nearestOk) guards.push(`anti-homonyme: nearest=${String(near.slug)} != ${cfg.slug} (override ${String(bcfg.identity_override)} non validé)`);
    if (grain !== "zone-polygon") guards.push(`grain classifié ${grain} != zone-polygon`);
    if (!countComplete) guards.push(`G2 fetch partiel/tronqué: ${String(entry.count_complete_basis)}`);
    if (served.hasRealV2ProofBlock || served.hasCollectionV2) guards.push(`servi porte DÉJÀ une preuve v2 → hors périmètre upgrade`);
    if (overlap !== null && overlap < OVERLAP_MIN_PCT) guards.push(`source-identity overlap ${overlap}% < ${OVERLAP_MIN_PCT}% → SKIP`);
    ...
    if (!commit) {
      ...
      entry.raison = `toutes gardes OK — prêt (champ=${chosen}, overlap ${String(overlap)}%, drop_empty=${droppedEmpty}, override=${overrideApplied ? bcfg.identity_override : "none"}, ${uncovered.length} servi-seul→UNKNOWN, grain=${grain})`;
      return entry;
    }
    // ── Dépôt (working = sous-ensemble servi) ──
    const norm = normalize(working as never[], chosen, url, "obscura-zone-vector");
    const proof = proofFromCaptureEntry(line, { type: proofType, method: "natif", reliability: "directe" });
    ...
    // Readback (contre le sous-ensemble servi)
    const capDigest = geometryDigest(working);
    ...
    for (const key of [flat, nested]) {
      ...
      const rb = {
        ...
        feature_count_matches_working: sfeats.length === working.length,
      ...
      readbacks.push(rb);
      const ok = rb.feature_count_matches_working && rb.geometry_digest_byte_exact && rb.zone_code_present_all && rb.proof_url_matches
    ...
    entry.raison = entry.readback_ok
      ? `upgrade legacy-traceable→documented v2 byte-exact ; zone_code = ${chosen} brut (overlap ${String(overlap)}%) ; grain=${grain} ; empty-drop=${droppedEmpty} ; override=${overrideApplied ? bcfg.identity_override : "none"} ; ${droppedCodes.length} code(s) servi-seul droppé(s)→UNKNOWN ; backup _replaced/ présent`
...
async function main(): Promise<void> {
  ...
  for (const cfg of cfgs) {
    const entry = await processMuni(s3, runs, registry, directory, cfg, commit);
  ...
  const record = {
    contract: "zones-vnatif-deposit-record-backlog/v1",
    date: "2026-08-10",
    ...
    decision: "CLEAN-UPGRADE BACKLOG (récupération file SKIP) : override anti-homonyme sur identité forte VÉRIFIÉE (wfs id==mamhCode / agol nom-exact) + drop empty-code borné (≤10% & overlap≥90%), candidate/legacy-traceable→documented v2",
    mode: commit ? "commit" : "dry-run",
    run_stamp: RUN_STAMP,
    worklist: worklistPath,
    empty_drop_max_frac: EMPTY_DROP_MAX_FRAC,
    gate: "depositCapturedZones identity gate PROVENANCE-AWARE + geometry_grain + anti-troncature (WFS numberMatched / ArcGIS returnCountOnly, sur téléchargement complet) + override d'identité vérifié + empty-drop borné",
