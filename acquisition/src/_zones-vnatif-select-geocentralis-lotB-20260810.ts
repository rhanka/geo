/**
 * _zones-vnatif-select-geocentralis-lotB-20260810.ts — SÉLECTION + validation LIVE
 * de la worklist LOT-B géoCentralis WFS (les ~40 munis suivants après le pilote + lot-A).
 *
 * Read-only vis-à-vis du servi : ne lit QUE le scoping (upgradable_list) et la worklist
 * lot-A (pour l'exclusion), puis PROBE la source WFS en direct (GetFeature&count=1 →
 * numberMatched) pour confirmer que chaque URL rend des features avant inclusion.
 *
 * Sélection :
 *   - host = geoserver.geocentralis.com (endpoint partagé unique)
 *   - exclut les 3 pilotes {adstock, baie-comeau, beauceville} + les 40 slugs lot-A
 *   - garde l'ordre du scoping (alphabétique) → prend les LIMIT (défaut 40) suivants
 *     dont l'URL rend numberMatched>0.
 *
 * Dérivation de l'URL (réplique EXACTE de la recette lot-A, cf. worklist lot-A) :
 ...
 *   Le rule est déduit à 100% de la worklist lot-A ; la validation LIVE numberMatched>0
 *   est le vrai garde (si la forme canonique rend 0, on essaie l'alternative ; sinon exclu).
 *
 * USAGE :
 *   NODE_OPTIONS=--dns-result-order=ipv4first AWS_MAX_ATTEMPTS=10 \
 *   npx tsx acquisition/src/_zones-vnatif-select-geocentralis-lotB-20260810.ts \
 *     [--limit 40] --worklist-out work/coverage/zones-vnatif-capture-worklist-geocentralis-lotB-20260810.json \
 *     --diag-out work/coverage/_zones-vnatif-select-geocentralis-lotB-20260810.json
 */
...
const LOTA_WORKLIST = "work/coverage/zones-vnatif-capture-worklist-geocentralis-lotA-20260810.json";
...
function loadLotASlugs(): Set<string> {
  const raw = JSON.parse(readFileSync(resolve(ROOT, LOTA_WORKLIST), "utf8")) as Array<{ slug: string }>;
  return new Set(raw.map((t) => t.slug));
}
...
/** Canonical CQL value per lot-A rule; alternate = other quoting. */
...
async function main(): Promise<void> {
  ...
  const lotA = loadLotASlugs();
  const candidates = scoping.filter((s) => s.url_host === HOST && !PILOT.has(s.slug) && !lotA.has(s.slug));
  process.stderr.write(`[select] geocentralis upgradable=${scoping.filter((s) => s.url_host === HOST).length} ; après exclusion pilote(${PILOT.size})+lotA(${lotA.size}) = ${candidates.length} candidats\n`);
  ...
  const record = { contract: "zones-vnatif-select-geocentralis-lotB/v1", date: "2026-08-10", host: HOST, excluded: { pilot: [...PILOT], lotA_count: lotA.size }, limit, candidates_total: candidates.length, selected: worklist.length, diag };
