/**
 * _zones-vnatif-deposit-geocentralis-pilot-20260810.ts — DÉPÔT v2 candidate/legacy-traceable
 * → documented du PILOTE géoCentralis WFS (3 munis, 2 couches partagées), grain zone-polygon.
 *
 * Prouve le chemin WFS → preuve v2 par source-identity de bout en bout avant tout batch.
 * Source = geoserver.geocentralis.com/geoserver/ows, GetFeature outputFormat=application/json
 * filtré par CQL_FILTER=id_municipalite=<id>, DÉJÀ CAPTURÉ sur le cluster (run 20260810T140000Z).
 *
 *   - adstock      evb:zonage_municipal   code=no_zonage_municipal  (ex. "M2.3-1")
 *   - baie-comeau  evb:siadmin_pzon_99_s  code=etiquette_1          (ex. "281 R")
 *   - beauceville  evb:zonage_municipal   code=no_zonage_municipal  (ex. "310-CN")
 *
 * IDENTITÉ DE SOURCE (sonde 05de001a) : le servi porte déjà le zone_source_url géoCentralis
 * (fragment déclaratif) au niveau legacy-traceable SANS preuve v2 (featureHasV2Proof=0) ; le
 * zone_code servi se REPRODUIT à 100% depuis le champ-code WFS (overlap mesuré, 0 code servi
 * non couvert). zone_code = valeur BRUTE du champ-code WFS (pas de dérivation synthétique).
 *
 * GARDES (par muni, avant tout dépôt) :
 *   G2  capture wfs/natif/directe, sha256 byte-exact (re-hash CAS == manifeste == clé CAS +
 *       verifyRawCapturePayload) ; complétude WFS = numberReturned==numberMatched==features
 *       (aucune troncature) ET énumération LIVE (count=1 → numberMatched) == features.
 *   grain par NATURE : PAS de marqueur UEV ⇒ zone-polygon (attendu). Sinon SKIP.
 *   anti-homonyme : nearest_registre_muni === slug.
 *   UPGRADE : le servi ne doit PAS déjà porter une vraie preuve v2 (legacy-traceable déclaratif).
 *   overlap ≥ 90% (source-identity). Sinon SKIP (HOLD).
 *   divergence : codes servi-seulement (attendu 0 ; overlap 100%) → gate PROVENANCE-AWARE les
 ...
 *
 * `--dry-run` (défaut) : resolve + G2 + grain + overlap + dropped SANS déposer.
 ...
 *   npx tsx acquisition/src/_zones-vnatif-deposit-geocentralis-pilot-20260810.ts \
 *     [--commit] [--only adstock] --out work/coverage/zones-vnatif-deposit-record-geocentralis-pilot-20260810.json
 */
...
const RUN_STAMP = "20260810T140000Z";
...
async function processMuni(
  ...
  const entry: Record<string, unknown> = {
    slug: cfg.slug, layer: cfg.layer, id_municipalite: cfg.id, code_field: cfg.codeField,
...
async function main(): Promise<void> {
  ...
  const cfgs = only ? MUNIS.filter((m) => m.slug === only) : MUNIS;
  ...
  const record = {
    contract: "zones-vnatif-deposit-record-geocentralis-pilot/v1",
    date: "2026-08-10",
    ...
    decision: "CLEAN-UPGRADE pilote géoCentralis WFS (legacy-traceable→documented v2), source-identity WFS, zone_code brut du champ-code",
