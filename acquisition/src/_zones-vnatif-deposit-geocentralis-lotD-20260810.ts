/**
 * _zones-vnatif-deposit-geocentralis-lotD-20260810.ts — DÉPÔT v2 candidate/legacy-traceable
 * → documented du LOT-D géoCentralis WFS (munis RESTANTS, 2 couches partagées), grain zone-polygon.
 *
 * Réplique EXACTE de la recette pilote (05de001a/cd363a04), lot-A (656eb46d), lot-B (dd7e4cf9)
 * et lot-C (bf4afa98) à l'échelle. Source = geoserver.geocentralis.com/geoserver/ows, GetFeature
 * outputFormat=application/json filtré par CQL_FILTER=id_municipalite=<id>, DÉJÀ CAPTURÉ sur le
 * cluster (run 20260810T180000Z).
 *
 * Les munis + URLs viennent de la worklist émise par la sonde de sélection lot-D (byte-exact :
 * l'URL de la worklist == l'URL rédigée du manifeste == l'URL de findLine). La couche est parsée
 * depuis typeNames= et mappée vers son champ-code :
 *   - evb:zonage_municipal   → no_zonage_municipal
 *   - evb:siadmin_pzon_99_s  → etiquette_1
 * zone_code = valeur BRUTE du champ-code (aucune dérivation synthétique).
 *
 * GARDES (par muni, isolation stricte : un muni KO = SKIP, jamais abort du lot) :
 *   G2  capture wfs/natif/directe, sha256 byte-exact (rehash CAS == manifeste == clé CAS +
 *       verifyRawCapturePayload) ; complétude WFS = numberReturned==numberMatched==features
 *       (anti-troncature) ET énumération LIVE (count=1 → numberMatched) == features.
 *   grain : PAS de marqueur UEV ⇒ zone-polygon (attendu). Sinon SKIP.
 *   anti-homonyme : nearest_registre_muni === slug.
 *   UPGRADE : le servi ne doit PAS déjà porter une vraie preuve v2.
 *   overlap ≥ 90% (source-identity). Sinon SKIP (HOLD, mauvaise-couche/ambigu).
 *   divergence : codes servi-seulement non-prouvés → gate PROVENANCE-AWARE UNKNOWN (jamais N-A) + backup.
 *   G5  servi après = géométrie octet-exacte de la capture, level→documented, url=proof.url,
 *       geometry_grain=zone-polygon ; backup _replaced/.
 *
 * `--dry-run` (défaut) : resolve + G2 + grain + overlap SANS déposer.
 * `--commit`           : dépose réellement.
 * `--only <slug>`      : restreint à un muni.
 * `--worklist f.json`  : worklist source (défaut lot-D).
 * `--out f.json`       : écrit le record.
 *
 * USAGE :
 *   NODE_OPTIONS=--dns-result-order=ipv4first AWS_MAX_ATTEMPTS=10 \
 *   npx tsx acquisition/src/_zones-vnatif-deposit-geocentralis-lotD-20260810.ts \
 *     [--commit] [--only <slug>] --out work/coverage/zones-vnatif-deposit-record-geocentralis-lotD-20260810.json
 */
...
const RUN_STAMP = "20260810T180000Z";
const GRAIN: GeometryGrain = "zone-polygon";
const OVERLAP_MIN_PCT = 90;
const DEFAULT_WORKLIST = "work/coverage/zones-vnatif-capture-worklist-geocentralis-lotD-20260810.json";
const CODE_FIELD_BY_LAYER: Record<string, string> = {
  "evb:zonage_municipal": "no_zonage_municipal",
  "evb:siadmin_pzon_99_s": "etiquette_1",
};

interface MuniCfg { slug: string; layer: string; codeField: string; url: string }
...
function loadWorklistCfgs(worklistPath: string): MuniCfg[] {
  ...
  for (const t of raw) {
    const url = t.urls[0]!;
    const ml = /typeNames=(evb:[a-z0-9_]+)/i.exec(url);
    const layer = ml ? ml[1]! : "";
    const codeField = CODE_FIELD_BY_LAYER[layer];
    if (!codeField) throw new Error(`worklist: couche inconnue pour ${t.slug} (url=${url})`);
    cfgs.push({ slug: t.slug, layer, codeField, url });
  }
  return cfgs;
}
...
// ── Source enumeration (LIVE) — WFS count=1 → numberMatched ────────────────────
...
async function liveNumberMatched(url: string): Promise<{ number_matched: number | null; error: string | null }> {
  try {
    const gj = await fetchJson(`${url}&count=1`) as { numberMatched?: unknown };
    return { number_matched: typeof gj.numberMatched === "number" ? gj.numberMatched : null, error: null };
  } catch (e) { return { number_matched: null, error: (e as Error).message }; }
}
...
async function processMuni(
  ...
  const entry: Record<string, unknown> = {
    slug: cfg.slug, layer: cfg.layer, code_field: cfg.codeField,
    run_stamp: RUN_STAMP, source_url: url, expected_geometry_grain: GRAIN, zone_code_source: `WFS ${cfg.codeField} (valeur brute)`,
  };
  try {
    ...
    // Parse WFS geojson
    const gj = JSON.parse(bytes.toString("utf8")) as { type?: string; features?: Feat[]; numberMatched?: unknown; numberReturned?: unknown; totalFeatures?: unknown };
    ...
    entry.is_featurecollection = isFC; entry.feature_count = feats.length;
    entry.wfs_number_matched = nmCap; entry.wfs_number_returned = nrCap; entry.wfs_total_features = gj.totalFeatures ?? null;
    ...
    // zone_code (valeur brute du champ WFS) + codes/nums
    const capCodes = new Set<string>(); let nullCode = 0; const sample: Record<string, unknown>[] = [];
    for (const f of feats) {
      const raw = f.properties?.[cfg.codeField];
    ...
    // WFS completeness + LIVE enumeration
    const live = await liveNumberMatched(url);
    entry.live_number_matched = live.number_matched; entry.live_enum_error = live.error;
    const noTruncation = nmCap !== null && nrCap !== null && nrCap === nmCap && feats.length === nmCap;
    const liveComplete = live.number_matched === null ? false : feats.length === live.number_matched;
    const countComplete = noTruncation && liveComplete;
    entry.count_complete = countComplete;
    entry.count_complete_basis = `capture numberReturned(${String(nrCap)})==numberMatched(${String(nmCap)})==features(${feats.length}) && live numberMatched(${String(live.number_matched)})==features`;
    // Served before
    ...
    if (nullCode > 0) guards.push(`${nullCode} feature(s) sans zone_code (champ ${cfg.codeField} vide) — anti-invention`);
    if (!countComplete) guards.push(`G2 fetch partiel/instable: ${String(entry.count_complete_basis)}`);
    ...
    if (overlap !== null && overlap < OVERLAP_MIN_PCT) guards.push(`source-identity overlap ${overlap}% < ${OVERLAP_MIN_PCT}% → SKIP (HOLD, mauvaise-couche/ambigu)`);
    ...
    if (!commit) {
      ...
      entry.raison = `toutes gardes OK — prêt au dépôt candidate→documented (overlap ${String(overlap)}%, ${uncovered.length} code(s) servi-seulement→UNKNOWN) (relancer avec --commit)`;
      return entry;
    }
    // ── Dépôt ──
    const norm = normalize(feats as never[], cfg.codeField, url, "obscura-zone-vector");
    const proof = proofFromCaptureEntry(line, { type: "wfs", method: "natif", reliability: "directe" });
    ...
    entry.raison = entry.readback_ok
      ? `upgrade legacy-traceable→documented v2 byte-exact ; zone_code = ${cfg.codeField} brut (overlap ${String(overlap)}%) ; grain=zone-polygon ; ${droppedCodes.length} code(s) droppé(s)→UNKNOWN ; backup _replaced/ présent`
...
async function main(): Promise<void> {
  ...
  const record = {
    contract: "zones-vnatif-deposit-record-geocentralis-lotD/v1",
    date: "2026-08-10",
    spec: "SPEC_ZONE_DEPOSIT_REPLACE_POLICY.md@64f82eae/65d4c637 + SPEC_ZONE_GEOMETRY_GRAIN.md@7c7f6731 + SPEC_ACQUISITION_METHODES_PAR_SOURCE.md@47fc8104 §12",
    decision: "CLEAN-UPGRADE lot-D géoCentralis WFS (legacy-traceable/candidate→documented v2), source-identity WFS, zone_code brut du champ-code (réplique pilote 05de001a/cd363a04 + lot-A 656eb46d + lot-B dd7e4cf9 + lot-C bf4afa98)",
    mode: commit ? "commit" : "dry-run",
    run_stamp: RUN_STAMP,
    worklist: worklistPath,
    proof_intended: { type: "wfs", method: "natif", reliability: "directe", schema_version: "2.0" },
