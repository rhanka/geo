/**
 * _zones-vnatif-resolve-lot3-20260810.ts — SONDE DIAGNOSTIC (lecture seule, aucun dépôt).
 *
 * Variante lot3 de _zones-vnatif-resolve-lot2-20260810.ts (run-stamp 20260810T121500Z,
 * 2 villes vecteur natif : saint-hippolyte, saint-lin-laurentides).
 *
 * Pour la capture cluster run-stamp 20260810T121500Z :
 *   1. liste `capture/_runs/zones-20260810T121500Z-*`, dérive les run_id (shards) ;
 *   2. pour chaque run VÉRIFIE le header (finished_at + exit_code 0) ;
 *   3. pour chaque URL de la worklist lot3, localise la ligne de capture (url|final_url)
 ...
 *   4. parse le geojson (FeatureCollection), compte les features, expose les types de
 *      géométrie et les champs de propriété candidats (zone_field) avec distinct/sample
 *      — pour choisir le zone_field réel (SKIP si code composite multi-champ).
 *
 * N'écrit RIEN sur S3. Émet un rapport JSON sur stdout.
 *
 * USAGE :
 *   NODE_OPTIONS=--dns-result-order=ipv4first AWS_MAX_ATTEMPTS=10 \
 *   npx tsx acquisition/src/_zones-vnatif-resolve-lot3-20260810.ts
 */
...
const RUN_STAMP = "20260810T121500Z";
const LANE = "zones";
const PREFIX = `capture/_runs/${LANE}-${RUN_STAMP}-`;
const WORKLIST = "work/coverage/zones-vnatif-capture-worklist-lot3-20260810.json";
...
// Registre municipal pour le discriminant anti-homonyme (nearest === slug).
...
function nearestMuni(feats: Array<{ geometry?: { coordinates?: unknown } | null }>, registry: MuniEntry[]): { slug: string | null; km: number | null } {
  ...
  return { slug: nearest?.slug ?? null, km: nearest ? Math.round(nearest.km * 100) / 100 : null };
}
...
async function main(): Promise<void> {
  ...
  for (const w of worklist) {
    ...
    // Parse geojson.
    let gj: { type?: string; features?: Array<{ properties?: Record<string, unknown> | null; geometry?: { type?: string; coordinates?: unknown } | null }> };
    try {
    ...
    const allPolygonal = geomTypes.size > 0 && [...geomTypes].every((t) => /Polygon/i.test(t));
    entry.all_polygonal = allPolygonal;
    // Anti-homonyme G4 : nearest registre muni doit == slug.
    const near = nearestMuni(feats, registry);
    entry.nearest_registre_muni = near.slug;
    entry.registry_attribution_km = near.km;
    entry.nearest_matches_slug = near.slug === w.slug;
    ...
    entry.status = feats.length > 0 && gj.type === "FeatureCollection" && allPolygonal ? "OK" : "EMPTY_OR_NOT_FC_OR_NONPOLY";
    report.push(entry);
  }
