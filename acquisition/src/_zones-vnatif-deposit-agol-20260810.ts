/**
 * _zones-vnatif-deposit-agol-20260810.ts — DÉPÔT v2 candidate/legacy-traceable →
 * documented du batch AGOL (ArcGIS Online hosted FeatureServers, host
 * services\d*\.arcgis.com), grain zone-polygon, preuve type=agol / natif / directe.
 *
 * Réplique EXACTE de la recette de dépôt VICTORIAVILLE (commit 5564b2f5) / ALTUS
 * (0b343380) — G2 byte-exact, grain, anti-homonyme, gate PROVENANCE-AWARE,
 * level→documented, url=proof.url, backup _replaced/, dropped→UNKNOWN (jamais N-A),
 * readback G5, anti-troncature ArcGIS (returnCountOnly[MÊME where]==features &
 * !exceededTransferLimit). SEULES DIFFÉRENCES avec victoriaville :
 *   (1) type de preuve = AGOL (FeatureServer hébergé ArcGIS Online).
 *   (2) TOPOLOGIE MIXTE : la worklist porte, PAR MUNI, soit un filtre `where=1=1`
 *       (couche per-muni), soit `where=<F>=<v>` (couche MRC PARTAGÉE filtrée par muni,
 *       F/v résolus au SELECT par nom + recouvrement ≥90% + anti-homonyme). Le comptage
 *       LIVE conserve LE MÊME `where` (countUrlFor) → numberReturned==numberMatched par
 *       partition. Aucune hypothèse figée sur le filtre : il vient de l'URL capturée.
 *   (3) CHAMP CODE-ZONE hétérogène par org (ZONE_, ZONE, Sect, Sect_label, NUM_ZONE,
 *       NO_ZONE, Usage…) — RÉSOLU par muni par recouvrement ≥90% avec le servi ; les
 *       champs muni-partition (MUN/MUNI/CODE_MUN/co_mun/Nom_Mun_View) et d'affectation
 *       sont exclus/écartés. zone_code = valeur BRUTE du champ résolu (jamais dérivé).
 *
 * `--dry-run` (défaut) : resolve + G2 + grain + champ + overlap SANS déposer.
 * `--commit`           : dépose réellement.
 * `--only <slug>`      : restreint à un muni.
 * `--worklist f.json`  : worklist source (défaut agol).
 * `--out f.json`       : écrit le record.
 *
 * USAGE :
 *   NODE_OPTIONS=--dns-result-order=ipv4first AWS_MAX_ATTEMPTS=10 \
 *   npx tsx acquisition/src/_zones-vnatif-deposit-agol-20260810.ts \
 *     [--commit] [--only <slug>] --out work/coverage/zones-vnatif-deposit-record-agol-20260810.json
 */
...
const RUN_STAMP = "20260810T210000Z";
const OVERLAP_MIN_PCT = 90;
const DEFAULT_WORKLIST = "work/coverage/zones-vnatif-capture-worklist-agol-20260810.json";
// Champs jamais éligibles comme code-zone (géométrie / id / audit / muni-partition / aires).
// Comparés en forme CANONIQUE (canonName = alnum-uppercase).
...
const KNOWN_CODE_NAMES = new Set<string>(["ZONE", "ZONAGE", "NUMZONE", "NOZONE", "SECT", "SECTLABEL", "USAGE"]);
...
// Comptage LIVE : conserve LE MÊME filtre `where` (1=1 OU F=v) que l'URL geojson capturée.
...
async function main(): Promise<void> {
  ...
  const record = {
    contract: "zones-vnatif-deposit-record-agol/v1",
    date: "2026-08-10",
    spec: "SPEC_ZONE_DEPOSIT_REPLACE_POLICY.md + SPEC_ZONE_GEOMETRY_GRAIN.md + SPEC_ACQUISITION_METHODES_PAR_SOURCE.md §12 ; réplique dépôt victoriaville 5564b2f5 / altus 0b343380",
    decision: "CLEAN-UPGRADE batch AGOL (legacy-traceable/candidate→documented v2), sources ArcGIS Online hosted FeatureServer, f=geojson, topologie MIXTE (per-muni where=1=1 OU couche MRC partagée filtrée F=v résolue par nom+recouvrement+anti-homonyme), champ code-zone résolu par recouvrement ≥90% avec le servi, zone_code brut",
