...
| 3 | `30-cap-billing-fn` — SA `billing.projectManager` **project-scope** + Function abonnée au topic | `describe` : state ACTIVE + eventTrigger=topic [couche 2 = le vrai hard-cap] | k8s/owner |
| 4 | `40-quota` — quota Map Tiles ~300/j (console) | quota posé [couche 3, ceinture secondaire] | k8s/owner |
| 5 | **`50-test-kill` (J) = LE GATE** — publie `{costAmount:51,budgetAmount:50}` → assert `billingEnabled=false` → owner ré-attache (argent, **owner-direct**) | **billing DÉTACHÉ prouvé** (poll borné) | k8s/owner |
...
**Gate global** : **FAIL** si un de 1–5 manque **OU** si J ne prouve pas `billingEnabled=false`.
...
1. Un garde-fou **A** échoue — **surtout J ne coupe pas** `billingEnabled=false` → STOP, pas de clé.
...
- **Owner-direct** (parole directe de l'owner dans la session exécutante, jamais un relais) : **billing-link (A1 + ré-attach A5)** + **création de clé (A6)**. `${BILLING_ACCOUNT}` fourni à l'exec, **jamais committé** (repo public).
