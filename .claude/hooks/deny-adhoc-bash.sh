#!/usr/bin/env bash
# =============================================================================
# PreToolUse / Bash gate for the geo project.
#
...
#     - permissions.defaultMode = "dontAsk" : if this hook is ever bypassed
#       (subagent path, missing $CLAUDE_PROJECT_DIR, hook error/timeout), the
#       baseline DENIES instead of PROMPTING. So a prompt is impossible.
#
...
#       and a stderr reason, then exit 0. We do NOT use exit 2 here because the
#       exit-0 JSON deny path is the mechanism already observed working in the
#       transcripts; the dontAsk backstop covers any version that ignores it.
