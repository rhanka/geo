...
2) ZONES — RECALAGE PDF COMPLET, méthode PROUVÉE sur les 30 focus (cf docs/spec/zonage-georeferencement-gcp.md). Cible: les slugs zones=to-research de ton shard qui ONT un plan PDF (buckets coverage-matrix: pdf-georef-t1 / pdf-vectorize-t2 / pdf-vectoriel-t2 / pdf-raster-t3 / pdf-scan-t4 / pdf-discovery-required). Récupère le plan de zonage PDF officiel de la muni puis ESCALADE dans cet ordre: (T1) détecte le géoréf EMBARQUÉ (/VP /Measure /GPTS via lib t1-georef); si présent → t1-build.ts (géométrie ligne-de-vue cadastre + labels pdftotext). (T2) sinon plan VECTORIEL sans géoréf → t2-autogcp.ts (match sommets du linework ↔ coins de lots cadastraux géoréférencés = VRAIS GCP indépendants, résidu ≤30m validé en holdout) puis t2-build.ts + OCR-labels dict-validés (t2-serve-vision / lib t2-labels). (T3) sinon plan RASTER/scan → t2-raster-register.ts (calage image ↔ cadastre rendu) + OCR-labels. Dépôt qc-zonage-<slug>. GATE STRICT: zone_code réglementaire RÉEL verbatim, gate spatial centroïde dans la BONNE muni, ≥3 codes lettrés, rejet séquentiel(OBJECTID/NO_ZONE numérique)/affectation(CMM/PMAD/SAD/milieux_humides)/bbox-stretch. NE JAMAIS servir un bbox-stretch comme réel. En complément seulement si plausible: zones-agol-owner-harvest.ts (gate spatial QC), zones-wfs-discover/run.
