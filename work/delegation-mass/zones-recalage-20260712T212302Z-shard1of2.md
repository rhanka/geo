# Recalage PDF zones — shard 1/2 — 2026-07-12T21:23:02.215Z

Règle: slugs dont `(index dans la liste triée) % 2 == 1`. Aucun AGOL owner harvest. Aucun code ou GCP inventé.

Résultat: **2 dépôts nets dans cette passe**, 135/135 slugs impairs non-done couverts par un dépôt ou une preuve d’échec structurée.

## Dépôts nets

- **chambord** — Deux plans officiels rural+urbain. Chaque feuillet: 10 GCP indépendants raster↔cadastre; rural résidu max 23,738 m / holdout 17,852 m, urbain 16,936 m / 13,916 m. Claude: 28+37 labels exacts validés contre 81 codes de la grille officielle. Fusion pooled: 65 labels, 64 codes distincts, 61 features, 2204/2377 lots (92,72%), centroïde 1,134 km, 0 hors bbox. Dépôt S3 puis lot-zone-join 92,76% et lots-enriched zone_code 92,76%, normes 90,24%. Source: https://chambord.ca/uploads/testzonage-rural-chambord.pdf ; https://chambord.ca/uploads/testzonage-pu-chambord.pdf
- **lac-superieur** — Plan officiel 2015-560. 13 GCP indépendants raster↔cadastre; résidu max 18,418 m / holdout 21,754 m. pdftotext: 168 codes candidats, 166 retenus verbatim contre le dictionnaire extrait du plan; E-04 et M-04 rejetés. 32 features, 66/66 lots (100%), centroïde 0,419 km, 0 hors bbox. Dépôt S3 puis lot-zone-join 100% et lots-enriched zone_code 100% (normes 0%, aucun registre de normes correspondant). Source: https://muni.lacsuperieur.qc.ca/wp-content/uploads/2023/10/2015-560-Plan-de-zonage-Annexe-A-m.a.j.-25-11-22.pdf

## Preuves fraîches d’échec

- **labrecque** — Deux plans officiels raster. À 72 DPI: territoire 6 GCP survivants (<8), urbain 5 (<8). À 144 DPI: territoire 9 GCP, résidu 27,063 m mais holdout 78,684 m > 30; urbain 5 GCP <8. Gate T3 ABORT, aucun dépôt.
- **matane** — T1 ABORT: aucun /VP /Measure /GEO. T2 page 2: 13 fits plausibles; désambiguïsation lot-assignment: tight 0% pour toutes les orientations, serving max 1,89%, marge 0 pt <15, seulement 2 codes; arbitrage anisotropie serving 0,26% <85%. SKIP strict, aucun GCP servi.
- **mayo** — T1 ABORT: aucun /VP /Measure /GEO. Le PDF officiel ne fournit que 16 points SVG exploitables; T2 affine et similarity n’obtiennent aucun seed résidu+holdout. Aucun dépôt.

## Vérifications

- `loop-supervise`: scoreboard zones=825; qualité 821 servis.
- Chambord: plans rural+urbain fusionnés, 2204/2377 lots, joins inline OK.
- Lac-Supérieur: 66/66 lots, joins inline OK; normes=0% rapporté honnêtement.
- La découverte officielle des lots 07 à 11 est conservée dans les JSON listés ci-dessus.
- 135/135 lignes courantes du shard ont une preuve; 0 hors shard; aucun `.claude`/`.track` touché par cette passe.

## Tous les slugs courants du shard
...
| 267 | labrecque | echec | T3 raster-register officiel, gate holdout | Deux plans officiels raster. À 72 DPI: territoire 6 GCP survivants (<8), urbain 5 (<8). À 144 DPI: territoire 9 GCP, résidu 27,063 m mais holdout 78,684 m > 30; urbain 5 GCP <8. Gate T3 ABORT, aucun dépôt. |
...
| 375 | matane | echec | T1 puis T2 auto-seed + lot-assignment | T1 ABORT: aucun /VP /Measure /GEO. T2 page 2: 13 fits plausibles; désambiguïsation lot-assignment: tight 0% pour toutes les orientations, serving max 1,89%, marge 0 pt <15, seulement 2 codes; arbitrage anisotropie serving 0,26% <85%. SKIP strict, aucun GCP servi. |
| 377 | mayo | echec | T1 puis T2 auto-seed affine/similarity | T1 ABORT: aucun /VP /Measure /GEO. Le PDF officiel ne fournit que 16 points SVG exploitables; T2 affine et similarity n’obtiennent aucun seed résidu+holdout. Aucun dépôt. |
