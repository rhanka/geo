# Recalage PDF zones — shard 1/2 — 2026-07-12T23:33:50.641Z

Règle: slugs dont `(index dans la liste triée) % 2 == 1`. Aucun AGOL owner harvest. Aucun code inventé.

Résultat consolidé: **1 dépôt net dans cette passe**, 135 slugs non terminés avec dépôt ou preuve, 0 fallback(s) sans preuve structurée.
...
| 65 | bonne-esperance | echec | preuve rapport antérieur | aucun site officiel ni plan PDF local trouvé dans le répertoire municipal; pas de source à servir sans invention |
| 69 | bowman | echec | preuve rapport antérieur | published candidate is a flat non-georeferenced PDF/PScript5 map; no official zoning-plan link recovered; T1/T2 cannot pass |
| 73 | brome | echec | preuve rapport antérieur | page officielle récupérée, aucun plan de zonage PDF; les PDF présents sont des règlements non cartographiques |
...
| 203 | gros-mecatina | echec | preuve rapport antérieur | official page read but no exploitable zoning plan PDF link |
...
| 235 | kazabazua | echec | preuve rapport antérieur | 20 puis 16 seeds résidu+holdout acceptés, mais aucun ne passe l'iso-gate orientation/isotropie; la reprise lot/aniso n'a produit ni GCP ni rapport |
...
| 303 | lanse-saint-jean | echec | preuve rapport antérieur | svg_points=326, aucun seed (extent × rotation) ne passe le résidu+holdout |
