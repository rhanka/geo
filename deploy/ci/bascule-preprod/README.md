...
| `rbac-ci-bascule-preprod.yaml` | SA `geo-ci-bascule-preprod` (ns geo-preprod) : Jobs + rollout geo-api, secrets get/update sur `geo-backup-reader-preprod` seulement, 0 logs. |
...
| --- | --- | --- |
| S0 | `preflight-backup` | binaires, params, format `BACKUP_ID`/`CYCLE_ID`, `PREPROD_DOCS` ≠ `PROD_DOCS`, endpoint figé |
| S0.s | `backup-secret-fill` | réécrit le Secret pré-créé `geo-backup-reader-preprod` depuis `GEO_BACKUP_READER_PREPROD_*` (gardes #405 : une ligne, `^[A-Za-z0-9]{16,128}$` / `^[A-Za-z0-9/+=]{16,128}$`, endpoint `https://s3.bhs.io.cloud.ovh.net`, `kubectl replace --dry-run=server` puis `replace`, jeu de clés vérifié ; valeurs par `env:` seulement, jamais en argv/log) |
...
| S3' | `docs-restore` | G3 ; état AU JOUR D du préfixe servi `normalized/` d'après `docs-inventory/<D>.json` (sha256 = manifeste) : copie **côté serveur** depuis `geo-backup/docs/<clé>?versionId=<v>` (version enregistrée, sinon celle dont l'ETag est celui de l'inventaire si l'objet a été réécrit depuis) ; `UploadPartCopy` au-delà de 5 GiB (lève la limite du point ouvert 2 pour ce mode) ; additif ; grant vers l'identité serving ; objet absent du backup ou sans version restaurable ⇒ refus avant la 1re copie |
...
`DRY_RUN=true` : S0 + S0.s + R0 + plan S3' (0 copie) + smoke informatif. Job `list` : S0 +
S0.s + Job de liste → journal, résumé du run, artefact `backup-list-geo-<CYCLE_ID|run_id>`
(`backup-list.json`, format `radar-backup-list/v1` attendu par l'orchestrateur immo).
...
| RBAC `geo-ci-bascule-preprod` | secrets get/update sur `geo-backup-reader-preprod` seulement (ni create/patch/list) — `rbac-ci-bascule-preprod.yaml` | à appliquer par k8s |
| Signataire S3' `BACKUP_DOCS_COPY_SECRET` (défaut `geo-backup-restore-docs`) | GetObject sur `geo-backup/docs/normalized/*` (+ listing des versions si possible) **et** PutObject + PutObjectAcl sur `sentropic-geo-preprod`, sans delete (modèle immo `radar-backup-restore-docs`) | **non provisionné** : sans lui S3' échoue (Secret absent). Alternative : étendre `geo-backup-reader-preprod` en écriture préprod et poser la variable `BASCULE_BACKUP_DOCS_COPY_SECRET=geo-backup-reader-preprod` |
