#!/usr/bin/env bash
# Runs INSIDE a Kubernetes Job (remote) — reads GeoJSON from S3, builds PMTiles
...
# under the 10Gi scratch budget), deleting each batch's GeoJSON after tiling. Best run AFTER province-wide
