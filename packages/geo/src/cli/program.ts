...
export interface ProgramDeps {
  ...
  /**
   * Source catalog (ADR-0017), injected once and handed to `sources list/show`
   * and `serve`. The bin builds it from the dynamically-loaded continent
   * registries; tests pass a fixture or rely on the empty default.
   */
...
export function buildProgram(deps: ProgramDeps = {}): Command {
  ...
  sources
    ...
    .action((opts: { country?: string; kind?: string }) => {
      ...
      out(formatSourceList(listSources(inventory, filters)));
    });
  sources
    .command("show <sourceId>")
    .description("Inspect a source from the catalog.")
    .action((sourceId: string) => {
      out(formatSourceDetail(showSource(inventory, sourceId)));
    });
  ...
  program
    ...
    .action((opts: { port?: number; data?: string }) => {
      ...
      // Inject the program-level inventory so `/sources` is populated, unless
      // the caller already supplied one in serveDeps.
      const serveDeps = { inventory, ...(deps.serveDeps ?? {}) };
      const handle = startServer(options, serveDeps);
