...
describe("program: sources show", () => {
  it("passes the sourceId argument through", async () => {
    const showSource = vi.fn((_inventory: unknown, id: string) => ({
      id,
      title: "SDA",
      kind: "administrative",
      jurisdiction: "CA-QC",
    ...
    // The program injects its (empty, in this test) inventory as the first arg.
...
describe("program: serve", () => {
  it("wires port/data to startServer", async () => {
    ...
    // The program forwards its (empty, in this test) inventory into serveDeps so
    // `/sources` is populated when continents are installed (ADR-0017, Risk #7).
    expect(startServer).toHaveBeenCalledWith({ port: 9000, data: "/data" }, { inventory: [] });
    expect(out.join("\n")).toContain("9000");
  });
});
