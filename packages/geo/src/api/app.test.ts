...
// The `/sources` catalog is injected (ADR-0017). Engine tests use the hermetic
// fixture inventory; the real catalog is covered in the continent libs' tests.
const INVENTORY = buildInventory([FIXTURE_REGISTRY]);
const app = createApp(makeProvider(), INVENTORY);
...
describe("sources catalog", () => {
  it("lists the full injected inventory", async () => {
    ...
    expect(body.numberMatched).toBe(FIXTURE_SOURCE_IDS.length);
    expect(body.numberReturned).toBe(FIXTURE_SOURCE_IDS.length);
    expect(body.sources).toHaveLength(FIXTURE_SOURCE_IDS.length);
