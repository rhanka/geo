/**
 * georef/geopdf.ts — pure-Node (no GDAL) extraction of a GeoPDF's embedded
 * georeferencing → a page→WGS84 transform (the T1 zoning recipe).
 *
 ...
 * does, with zero native deps — only `proj4` and pure-Node stream inflate.
 *
 ...
 * The shared affine solver ({@link fitAffine}, the {@link GeoRef} contract) and the
 * WKT→proj4 parser ({@link wktToProj4}) are reused from this same `georef` module.
 * The `pdfinfo` page-size probe is an OPTIONAL enhancement (only when a `pdfPath`
 * is supplied); without it the page MediaBox is read straight from the inflated
 * text, so `extractGeoRef(buffer)` is a pure buffer→GeoRef transform.
 */
