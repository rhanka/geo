/**
 * georef/gcp.ts — manual N-point (≥3) georeferencing from human/derived Ground
 * Control Points, the pure-compute contract shared by every GCP-driven georef.
 *
 * A plan (AutoCAD / PScript / Acrobat export) may carry zone-code labels but NO
 * embedded geographic registration — only `/Subtype/RL` rectilinear scale-bar
 * measures (CAD units), which have no CRS anchor. This module rebuilds the SAME
 * {@link GeoRef} contract as the embedded-GeoPDF path from a handful of GCPs:
 * a page fraction ↔ its real-world [lon,lat]. With ≥3 spread GCPs it fits an
 * affine page→WGS84 transform (least squares, the exact same {@link fitAffine}
 * solver as the embedded path) and exposes `pageToLonLat` / `topLeftToLonLat`.
 *
 ...
 * native deps. ZERO geometry or codes are invented: the GCPs only register WHERE
 * existing labels land on Earth.
 *
