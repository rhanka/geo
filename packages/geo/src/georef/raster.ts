/**
 * georef/raster.ts — pure raster image primitives for cadastre↔plan registration.
 *
 * The raster sibling of the vector georef path: to register a rendered zoning
 * plan against rendered cadastre lot edges you need to (1) decode the grayscale
 * raster, (2) extract an edge mask, (3) find repeatable corner features, and
 * (4) score how well a plan edge-patch matches a reference edge-patch. Those four
 * steps are pure pixel math and live here; the pdftoppm render, the cadastre-
 * reference rasterisation and the seed/residual/holdout matching loop
 * (`deriveRasterRegistration`) stay in the acquisition app layer.
 *
 * Ported (pure compute, zero I/O) from the acquisition recalage pipeline
 * (`acquisition/src/lib/t2-raster-register.ts`). Uses only the {@link NeatlineFrac}
 * page-fraction rectangle from this same `georef` module.
 */
...
/** Decode a binary (P5) or ASCII (P2) PGM into an 8-bit grayscale image. */
...
/** Sobel-gradient + dark-pixel edge mask (1 = edge). */
...
/**
 * Harris-response corner detection over the edge mask, restricted to the optional
 * neatline, with per-cell non-maximum suppression and a min-distance filter.
 */
...
/**
 * Dice-overlap of two (1-px-dilated) edge patches over a small shift search
 * (best of ±maxShift). 1 = identical edges, 0 = no overlap; below 8 edge pixels
 * on either side the shift is skipped (too sparse to judge).
 */
