/**
 * Unit tests for the auto-seed orientation / isotropy gate (georef/gate.ts).
 *
 * Pure, network-free. The gate is the anti-invention safety net that stops an
 * auto-derived georeferencing serving a residual-clean but GEOMETRICALLY WRONG
 * affine (stretched, mirrored, or rotated/flipped). We:
 ...
 *
 * Ported verbatim (import paths only) from the acquisition golden test
 * `acquisition/src/lib/t2-autogcp.test.ts`.
 */
import { describe, it, expect } from "vitest";

import { decomposeGcpAffine, decomposeGcpSimilarity, evaluateAffineGate } from "./gate.js";
